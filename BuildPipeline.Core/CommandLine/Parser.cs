...
namespace BuildPipeline.Core.CommandLine
{
    ...
    public class Parser
    {
        ...
        public static TypeInfo GetTypeInfo(object target, object defaultObject)
        {
            ...
            if(target.GetType().IsImplementFrom<ICustomTypeDescriptor>())
            {
                return new TypeInfo(target as ICustomTypeDescriptor, defaultObject);
            }
        ...
        /// <param name="arguments">The arguments.</param>
        /// <returns>ParserResult&lt;T&gt;.</returns>
        public ParserResult<T> Parse<T>(IEnumerable<string> arguments) where T : class, new ()
        {
        ...
        /// <returns>ParserResult&lt;T&gt;.</returns>
        public ParserResult<T> Parse<T>(string arguments, T value) where T : class
        {
        ...
        /// <param name="value">The value.</param>
        /// <returns>ParserResult&lt;T&gt;.</returns>
        public ParserResult<T> Parse<T>(IEnumerable<string> arguments, T value) where T : class
        {
        ...
        private object GetValues(object value, TypeInfo typeInfo, PropertyInfo property, List<string> values, ref IParserResult result)
        {
            if(property.IsBindingList)
            {
                IBindingList list = Activator.CreateInstance(property.Type) as IBindingList;
        ...
        private static string[] GetValueString(PropertyInfo propertyInfo, object value)
        {
            if(value == null)
            {
                return null;
        ...
        private static string GenUsageHelp(PropertyInfo property)
        {
            // this is flags enum
            if(property.IsFlags)
            {
            ...
            // array support
            else if(property.IsBindingList)
            {
